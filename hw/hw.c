...
void remove_page_LRU(struct Object main_mem[MAIN_PAGES][MAX_ADDR], struct Object virt_mem[VIRT_PAGES][MAX_ADDR],
          struct Page page_table[VIRT_PAGES], int page)
{
    if (len > 0)
    {
        int i;
        ...
        int dpn = LRU[i];
        int pos = i;
        int ppn = page_table[dpn].page_num;
        page_table[dpn].dirty_bit = false;

        for(i = 0; i < 8; i++) 
            virt_mem[dpn][i].value = main_mem[ppn][i].value;
...
void remove_page(struct Object main_mem[MAIN_PAGES][MAX_ADDR], struct Object virt_mem[VIRT_PAGES][MAX_ADDR],
          struct Page page_table[VIRT_PAGES])
{
    if (len > 0)
    {
        int pnn = page_table[FIFO[0]].page_num;
        page_table[FIFO[0]].dirty_bit = false;
        int i;
        for(i = 0; i < 8; i++)
            virt_mem[FIFO[0]][i].value = main_mem[pnn][i].value;
          ...
          struct Page page_table[VIRT_PAGES], int *main_len, int addr)
{
    ...
    if (!page_table[page].valid_bit)
    {
        ...
        else
        {
            if (pr_algo)
            {
                ...
                //page_table[dpn].page_num = dpn;

                // Get the new open spot in main
                int pnn = page_table[dpn].page_num;
                remove_page(main_mem, virt_mem, page_table);
                insert_page(page);
            ...
            else
            {
                ...
                // Get the new open spot in main
                int pnn = page_table[dpn].page_num;
                remove_page_LRU(main_mem, virt_mem, page_table, LRU[0]);
                insert_page(page);
    ...
    {
        if(!pr_algo) {
            remove_page_LRU(main_mem, virt_mem, page_table, page);
            insert_page(page);
        }
           ...
           struct Page page_table[VIRT_PAGES], int *main_len, int addr, int num)
{
    ...
    if (!page_table[page].valid_bit)
    {
        ...
        if ((*main_len) < 4)
        {
            ...
            main_mem[*main_len][offset].value = num;
        ...
        else
        {
            if (pr_algo)
            {
                ...
                // Removing 'first in' from main memory
                // Set page_num back to its virtual page num
                page_table[dpn].valid_bit = false;
                page_table[dpn].dirty_bit = false;
                // page_table[dpn].page_num = dpn;
                ...
                // Copy the page of the specified address from virtual memory
                int i;
                for (i = 0; i < MAX_ADDR; i++)
                    main_mem[pnn][i].value = virt_mem[page][i].value;
